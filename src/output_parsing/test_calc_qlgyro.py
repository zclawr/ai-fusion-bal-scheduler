...
def get_sat_params(sat_rule_in, ky, gammas, mts=5.0, ms=128, small=0.00000001, **kw):
    ...
    for m in range(0, ms + 1):
        ...
        b_geo[m] = B[m]
        costheta_geo[m] = (
            -Rmaj_s * (Bp[m] / (B[m]) ** 2) * (Bp[m] / r_curv[m] - (f**2 / (Bp[m] * (R[m]) ** 3)) * sin_u[m])
        )
    ...
    if units == "GYRO" and sat_rule_in == 1:  # new
        SAT_geo1_out = 1.0
        SAT_geo2_out = 1.0
...
def intensity_sat(
    ...
    **kw,
    ...
    if sat_rule_in == 3:
        ...
        for k in range(1, nmodes + 1):
            ...
            if kmax <= ky_spect[0]:
                ...
                i = 1
                # Loop until ky_spect[i-1] is no longer less than kmax
                while ky_spect[i - 1] < kmax:
                    i += 1

                # Check if either sum_W_i at the current or previous index is zero
                if sum_W_i[i - 1] == 0.0 or sum_W_i[i - 2] == 0.0:
                    x = 0.5
                else:
                    # Calculate the absolute value of the ratio of QL_flux_spectrum_out to sum_W_i
                    abs_W_ratio = np.abs(QL_data[:, k - 1, 0, 0, 1] / sum_W_i)
                    abs_W_ratio = np.nan_to_num(
                        abs_W_ratio
                    )  # Replace NaNs with zero and infinities with large finite numbers
                    x = linear_interpolation(ky_spect, abs_W_ratio, kmax)

            # Store the interpolated or default value of x in xs for the k-th index
            xs[k - 1] = x
    ...
    for j in range(0, nky):
        kx = kx0_e[j]
        if sat_rule_in == 2 or sat_rule_in == 3:  ##new
            ky0 = ky_spect[j]
            if ky0 < kycut:
    ...
    for j in range(jmax1 + 2, nky):
        ...
        for i in range(jmax1 + 1, nky - 1):
            ...
            ky_2 = ky_spect[i + 1]
            mix1 = ky_spect[j] * (
                np.arctan(sqcky * (ky_2 / ky_spect[j] - 1.0)) - np.arctan(sqcky * (ky_1 / ky_spect[j] - 1.0))
            )
        ...
        if ky_spect[-1] >= kT:
            ...
            for i in range(k - 1, k + 1):
                ...
                if ky0 < kycut:
                    ...
                    sat_geo_factor = (
                        kw["SAT_geo0_out"]
                        * (d1 * kw["SAT_geo1_out"] * kycut + (ky0 - kycut) * d2 * kw["SAT_geo2_out"])
                        / ky0
                    )
                    ...
                    if ky0 <= kT:
                        ...
                        if kP >= kT:
                            ...
                            field_spectrum_out[j, i - 1] = (
                                Ys[i - 1] * (sig_ratio**c_1) * Fky * (gp[j, i - 1] / gamma0) ** (2 * expsub)
                            )
                            ...
                            foversig0 = (
                                ((Ys[i - 1] / YTs[i - 1]) ** (1.0 / abs(c_1))) - eoversig0 * kT - doversig0 * (kT**2)
                            )
                        ...
                        field_spectrum_out[j, i - 1] = (
                            scal * measure * cnorm * ((gammaeff / (kx_width * ky0)) / (1.0 + ay * kx**2)) ** 2
                        )
                        if units_in != "GYRO":
...
def calculate_QLGYRO_flux(runid, cgyro_freq, cgyro_ql, tglf_growth, sat_rule_in, alpha_zf_in):
...
def calculate_TGLF_flux(runid, tglf_growth, cgyro_ql, tglf_ql, sat_rule_in, alpha_zf_in):
    ...
    if sat_rule_in == 1:
        delta = 0
        intensity_tglf, QLA_P, QLA_E, QLA_O = array(
            intensity_sat(1, tglf_kys, tglf_gammas.T, kx0_e, 1, QL_data, **in0)
        ).T
    if sat_rule_in == 2:
        delta = 0
        intensity_tglf, QLA_P, QLA_E, QLA_O = array(
            intensity_sat(2, tglf_kys, tglf_gammas.T, kx0_e, 1, QL_data, **in0)
        ).T
    if sat_rule_in == 3:
        delta = 0
        intensity_tglf, QLA_P, QLA_E, QLA_O = array(
            intensity_sat(3, tglf_kys, tglf_gammas.T, kx0_e, 1, QL_data, **in0)
        ).T
    ...
    tglf_sat = sum_ky_spectrum(
        ...
        tglf_kys,  #
        tglf_gammas.T,  #
        np.zeros(shape(tglf_kys)),  #
        R_unit,  #
        kx0_e,  # spectral shift
        np.zeros(shape(tglf_gammas.T)),
        particle_QL,
        energy_QL,
        toroidal_stress_QL,
        parallel_stress_QL,
    ...
    heat_flux_spectrum = np.sum(heat_flux_spectrum, axis=1)
    return (tglf_satG, tglf_satQ, tglf_satP, tglf_satPara, tglf_satExch, heat_flux_spectrum, tglf_sum_flux_spectrum)


def load_ql_fluxes_for_qlgyro(root_dir, apply_bt_sign=True):
    ky_dirs = sorted([
    ...
    for d in ky_dirs:
        try:
            ...
            fluxes = gyro["qlflux_ky"]
            freq = gyro["freq"]
            ky = freq["ky"].values.item()

            data = np.stack([
                np.sum(fluxes["particle"].values, axis=1),
                np.sum(fluxes["energy"].values, axis=1),
                np.sum(fluxes["momentum"].values, axis=1),
            ], axis=-1)  # shape: (species, field)

            data = data[:, -1, :]  # take last mode

            # Format QL data as xarray for compatibility with calculate_QLGYRO_flux
            ql_weight = xr.DataArray(
                data[np.newaxis, :, :, np.newaxis],  # (ky=1, species, moment, mode=1)
                dims=["ky", "species", "moment", "mode"],
                coords={
                    "ky": [ky],
                    "species": fluxes['particle'].coords['species'].values,
                    "moment": ["particle", "energy", "momentum"],
                    "mode": [0]
                }
            )

            ql_weight = ql_weight.roll(species=1, roll_coords=True)
            ql_weight.loc[dict(mode=1)] = 0.0  # Fill subdominant mode

            all_freq_data.append(freq)
            all_ql_weights.append(ql_weight)
            valid_runids.append(os.path.basename(d))

        except Exception as e:
            print(f"Skipping {d} due to error: {e}")

    return valid_runids, all_freq_data, all_ql_weights
...
def load_cgyro_simulations_modified(root_dir, input_file_path):
    in0 = parse_tglf_input_file(input_file_path)
    ...
    for sub in sorted(os.listdir(root_dir)):
        ...
        # Extract ky and gamma from CGYRO
        freq_da = gyro['freq']['gamma'].rename({"t": "gamma"})
        ky = freq_da.coords['ky'].values.item()  # scalar ky per run
        gamma = freq_da.isel(gamma=-1).values.item()  # final gamma

        if gamma <= 0.0:
            print(f"Skipping {sub} due to non-positive gamma = {gamma}")
            continue
        ...
        if nspecies is None:
            ...
            raise ValueError(f"Inconsistent number of species in {sub}")

        ql_weights = np.zeros((nmodes, ns, nfields))
        for imom, mom in enumerate(moments):
            flux_mom = fluxes[mom]  # shape: (species, field)
            ql_weights[0, :, imom] = flux_mom.values[:, -1]  # use last field component

        kys_all.append(ky)
        gamma_all.append(gamma)
        ql_list.append(ql_weights)


    # Convert lists to arrays
    kys_all = np.array(kys_all)
    gamma_all = np.array(gamma_all)
    ql_array = np.stack(ql_list, axis=0)  # shape: (nky, 1, nspecies, nfields)

    ql_da = xr.DataArray(
        ql_array,
        dims=["ky", "mode", "species", "moment"],
        coords={
            "ky": kys_all,
            "mode": [0],
...
sims = load_cgyro_simulations_modified("/global/homes/w/wyl002/Github/ai-fusion-gknn-wesley/all_kys", "/global/homes/w/wyl002/Github/ai-fusion-gknn-wesley/envs/output_parsing/test_calc_qlgyro.py")

for sim in sims:
    G, Q, P, heat_spec, spec_sum = calculate_QLGYRO_flux(
        runid=sim['name'],
        cgyro_freq=sim['freq'],
        cgyro_ql=sim['ql'],
        tglf_growth=None,
        sat_rule_in=int(sim['inputs']['SAT_RULE']),
        alpha_zf_in=float(sim['inputs']['ALPHA_ZF']),
        inputs=sim['inputs']  # <-- you'll pass this into the function now
    )

out_dir = "/global/homes/w/wyl002/Github/ai-fusion-gknn-wesley/all_kys"